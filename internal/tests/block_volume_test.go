...
func TestLocalWithSBSVolume(t *testing.T) {
	...
	tester.Test(t, &tester.TestConfig{
		Config: fmt.Sprintf(`
			...
			`, zone, imageName, rootVolumeType, rootVolumeSize, blockVolumeSize),
		Checks: []tester.PackerCheck{
			checks.InstanceVolume(zone, "Ubuntu").
				SizeInGB(uint64(rootVolumeSize)),
			checks.BlockVolume(zone, "packer-").
				//Name(volumeName).
				IOPS(15000).
			...
			checks.Image(zone, imageName).
				...
				ExtraVolumeSnapshot("1", checks.BlockSnapshot(zone, imageName).
					SizeInGB(uint64(blockVolumeSize)),
				),
		},
		Cleanup: []tester.PackerCleanup{
			...
			cleanup.BlockVolume(zone, "packer-"), // TODO: prefixes should be consts
			cleanup.InstanceVolume(zone, "Ubuntu"),
		},
	})
}

// Test multiple block volumes + check volume specs
// Almost OK, see TODO
func TestBlockOnly(t *testing.T) {
	zone := scw.ZoneFrPar1
	imageName := "packer-e2e-block"
	volumeName := "packer-e2e-block-vol1"
	volumeSize := 20
	...
	tester.Test(t, &tester.TestConfig{
		...
		Checks: []tester.PackerCheck{
			checks.BlockVolume(zone, "Ubuntu").
				SizeInGB(uint64(10)),
			checks.BlockVolume(zone, imageName).
				Name(volumeName).
				IOPS(5000).
				SizeInGB(uint64(volumeSize)),
			checks.Image(zone, imageName).
				//SizeInGB(uint64(30)).   // TODO: fix size = 0
				RootVolumeSnapshot(checks.BlockSnapshot(zone, imageName).
					SizeInGB(uint64(10))).
				ExtraVolumeSnapshot("1", checks.BlockSnapshot(zone, imageName).
					SizeInGB(uint64(volumeSize)),
				),
		...
		Cleanup: []tester.PackerCleanup{
			cleanup.Image(zone, imageName),
			cleanup.BlockSnapshot(zone, imageName),
			cleanup.BlockVolume(zone, imageName),
			cleanup.BlockVolume(zone, "Ubuntu"),
		},
	})
}
